...
/*   Updated: 2026/01/19 13:34:48 by jde-orma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
Span::Span( const Span &oneSpan ) : _maxSize(oneSpan.getN()), _numbers(oneSpan._numbers) {
...
void	Span::addSequence( VectorIt start, VectorIt end ) {
	...
	if (_numbers.size() + dist > _maxSize) {
		throw std::out_of_range("Error: Adding sequence would exceed capacity.");
	}
